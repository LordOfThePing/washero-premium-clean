...
export function resolveBotmakerRuleNameOrId(
  ...
  if (key === "bank_transfer_info") {
    const envRule = (process.env.BOTMAKER_WA_TEMPLATE_BANK_TRANSFER_INFO ?? "").trim();
...
export function sanitizeForLog(value: unknown, depth = 0): unknown {
  ...
  const token = process.env.BOTMAKER_API_TOKEN ?? "";
...
export async function sendBotmakerWhatsApp(
  ...
  const baseUrl = (process.env.BOTMAKER_BASE_URL ?? DEFAULT_BASE).replace(/\/$/, "");
  const sendPath = process.env.BOTMAKER_SEND_PATH ?? DEFAULT_SEND_PATH;
...
function resolveTemplateSendConfig() {
  const sendMode = (process.env.BOTMAKER_TEMPLATE_SEND_MODE ?? DEFAULT_TEMPLATE_SEND_MODE)
    .trim()
    .toLowerCase();
  const sendPath = process.env.BOTMAKER_TEMPLATE_SEND_PATH ?? DEFAULT_TEMPLATE_SEND_PATH;
  const templateBaseUrl = (
    process.env.BOTMAKER_TEMPLATE_BASE_URL ??
    process.env.BOTMAKER_BASE_URL ??
    DEFAULT_TEMPLATE_BASE
  ).replace(/\/$/, "");
  const channelId = (process.env.BOTMAKER_CHANNEL_ID ?? "").trim();
  const chatChannelNumber = (process.env.BOTMAKER_CHAT_CHANNEL_NUMBER ?? "").trim();
