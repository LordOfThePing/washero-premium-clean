...
const SKIP_PAYMENT = (process.env.WASHERO_SKIP_PAYMENT ?? "").toLowerCase() === "true";
...
async function createMercadoPagoCheckout(booking: {
  ...
  const WEBHOOK_URL = `${process.env.SUPABASE_URL}/functions/v1/mercadopago-webhook`;
...
export function validateBotmakerToolsAuth(req: Request): boolean {
  const secret =
    process.env.BOTMAKER_BOOKING_TOOLS_SECRET ??
