...
function isAdjacentTime(slotStartMin: number, bookingStartMin: number, bookingDuration: number): boolean {
  const bookingEnd = bookingStartMin + bookingDuration;
  const gapBefore = slotStartMin - bookingEnd;
  const gapAfter = bookingStartMin - slotStartMin;
  if (gapBefore >= 0 && gapBefore <= 90) return true;
  if (gapAfter >= 0 && gapAfter <= 90) return true;
  return false;
}
...
export function scoreLogisticSlot(
  ...
  for (const b of activeBookings) {
    ...
    if (typeof blat === "number" && typeof blng === "number") {
      ...
      if (bestDistanceKm === null || dist < bestDistanceKm) bestDistanceKm = dist;

      if (dist <= 3) {
        score += 40;
        nearbyBookingsCount++;
        reason = "Recomendado: ya tenemos un lavado cerca";
      } else if (dist <= 7) {
        score += 20;
        if (reason === "Disponible") reason = "Recomendado: zona con lavados cercanos";
      } else if (dist <= 12) {
        score += 10;
      } else {
        farJumpPenalty += 12;
    ...
    const adjacent = isAdjacentTime(slotStart, bStart, b.duration_minutes ?? 60);
    const closeEnough = bestDistanceKm !== null && bestDistanceKm <= 7;
    if (adjacent && (sameZone || closeEnough)) {
      score += 30;
      if (reason === "Disponible" || reason === "Recomendado: zona con lavados cercanos") {
        reason = "Recomendado: encadenado con otro lavado cercano";
      }
    }
  }

  score -= farJumpPenalty;
  if (farJumpPenalty > 0 && reason === "Disponible") {
    reason = "Disponible, pero con mayor traslado";
  }
  if (zoneMatch && reason === "Disponible") {
    reason = "Recomendado: misma zona";
  }
...
export function splitRecommendedSlots(
  ...
  for (const s of sorted) {
    if (s.score >= minScore) recommendedIds.add(s.slot_id);
  }
