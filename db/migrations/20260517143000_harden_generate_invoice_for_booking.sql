-- Harden generate_invoice_for_booking: safe role gate + restrict EXECUTE.
-- Allows service_role (edge functions / webhooks) or authenticated admins only.
...
DECLARE
  ...
  v_line_items jsonb;
BEGIN
  -- service_role (Supabase service key / server) OR active admin; never infer from missing JWT
  IF NOT (
